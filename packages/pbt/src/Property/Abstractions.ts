...
export type Property<Ts extends any[]> = {
  run(seed: number, iterations: number, config?: PropertyConfig): Iterable<Property.PropertyIteration<Ts>>;
};
...
export namespace Property {
  ...
  export namespace PropertyFunction {
    export const invoke = <Ts extends any[]>(f: PropertyFunction<Ts>, values: Ts): PropertyFunctionInvocation => {
      try {
        return f(...values) === false
          ? PropertyFunctionInvocation.returnedFalse()
          : PropertyFunctionInvocation.success();
      } catch (error) {
