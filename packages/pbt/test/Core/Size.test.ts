...
test.property('increments by one', Gen.integer().between(0, 98).growBy('constant'), (initialSize) => {
  expect(dev.Size.increment(initialSize)).toEqual(initialSize + 1);
});

test.property(
  'increments by ten, for a big increment',
  Gen.integer().between(0, 89).growBy('constant'),
  (initialSize) => {
